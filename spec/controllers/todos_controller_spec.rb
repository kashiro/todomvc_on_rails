...
RSpec.describe TodosController, :type => :controller do
  ...
  describe ":toggle action" do
    ...
    it "should toggle completed attribute of the todo" do
      expect(@todo.completed).to be false
      post :toggle, :id => @todo
      expect(Todo.find(@todo.id).completed).to be true
    end
  end
  ...
  describe ":toggle_all action" do
    ...
    it "should toggle completed attribute of all todos" do
      ...
      post :toggle_all
      expect(Todo.completed.count).to eq(0)
    end
  end
  ...
  describe ":destroy_completed action" do
    ...
    it "should delete all completed todo" do
      ...
      expect(Todo.completed.count).to eq(10)
      delete :destroy_completed
      expect(Todo.completed.count).to eq(0)
    end
  end
  ...
  describe ":destroy action" do
    ...
    it "should delete todo" do
      expect(Todo.where("id = ?", @todo.id).count).to eq(1)
      delete :destroy, :id => @todo
      expect(Todo.where("id = ?", @todo.id).count).to eq(0)
